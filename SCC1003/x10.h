...
class x10Class //: public Stream
{
  public:
   ...
   void write(byte houseCode, byte numberCode, byte numRepeats);			// sends a command
   
   uint8_t EncodeX10House(uint8_t EncodeValue);
   uint8_t EncodeX10Unit(uint8_t EncodeValue);
   uint8_t EncodeX10State(uint8_t EncodeValue);
  ...
  private:
    ...
    //void write(byte houseCode, byte numberCode);			// sends a command
